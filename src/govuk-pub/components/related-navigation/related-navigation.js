...
function RelatedNavigation ($module) {
  this.module = $module
}

RelatedNavigation.prototype.init = function () {
  var $module = this.module
  if (!$module) {
    return
  }
  var toggleSelector = '[data-controls][data-expanded]';
  var toggleClass = $module.attr('data-toggle-class') || 'js-hidden';

  $module.on('click', toggleSelector, toggle);
  $module.find(toggleSelector).each(addAriaAttrs);
}

RelatedNavigation.prototype.addAriaAttrs = function () {
  var $toggle = $(this);
...
RelatedNavigation.prototype.toggle = function (event) {
...
RelatedNavigation.prototype.getTargetElements = function ($toggle) {
