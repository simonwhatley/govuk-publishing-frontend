...
(function (Modules) {
  ...
  Modules.StepByStepNavigation = function () {
    ...
    var activeLinkClass = 'govuk-pub-step-nav__list-item--active'
    var activeStepClass = 'govuk-pub-step-nav__step--active'
    var activeLinkHref = '#content'
    var uniqueId

    this.start = function ($element) {
      // Indicate that js has worked
      $element.addClass('govuk-pub-step-nav--active')

      // Prevent FOUC, remove class hiding content
      $element.removeClass('js-hidden')

      stepNavSize = $element.hasClass('govuk-pub-step-nav--large') ? 'Big' : 'Small'
      ...
      var totalLinks = $element.find('.govuk-pub-step-nav__link').length
      ...
      function addShowHideAllButton () {
        $element.prepend('<div class="govuk-pub-step-nav__controls"><button aria-expanded="false" class="govuk-pub-step-nav__button govuk-pub-step-nav__button--controls js-step-controls-button">' + actions.showAllText + '</button></div>')
      }
        ...
        $stepHeaders.each(function () {
          ...
          if (!$(this).find('.js-toggle-link').length) {
            $(this).find('.js-step-title-button').append(
              '<span class="govuk-pub-step-nav__toggle-link js-toggle-link" aria-hidden="true" hidden></span>'
            )
          }
        })
      }
      ...
      function addButtonstoSteps () {
        $.each($steps, function () {
          ...
          $title.wrapInner(
            '<button ' +
            'class="govuk-pub-step-nav__button govuk-pub-step-nav__button--title js-step-title-button" ' +
      ...
      function setActiveStepClass () {
        ...
        $element.find('.' + activeLinkClass).closest('.govuk-pub-step-nav__step').addClass(activeStepClass).attr('data-show', '')
      }
    ...
    function componentLinkClick (event, stepNavTracker, linkPosition) {
      ...
      function trackClick () {
        ...
        var dimension28 = $(event.target).closest('.govuk-pub-step-nav__list').attr('data-length')
